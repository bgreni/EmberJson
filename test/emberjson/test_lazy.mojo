...
# emberserde entry point. `from_json` builds the same `Parser` over
...
def test_lazy_int() raises:
    # Test simple int
    var single = "123"
    var s_single = from_json[LazyInt[origin_of(single)]](single)
    assert_equal(s_single.get(), 123)

    # Test negative int
    var negative = "-42"
    var s_negative = from_json[LazyInt[origin_of(negative)]](negative)
    assert_equal(s_negative.get(), -42)

    # Test large int
    var large = "9007199254740991"
    var s_large = from_json[LazyInt[origin_of(large)]](large)
...
def test_lazy_uint() raises:
    # Test simple uint
    var single = "123"
    var s_single = from_json[LazyUInt[origin_of(single)]](single)
    assert_equal(s_single.get(), 123)

    # Test large uint
    var large = "18446744073709551615"
    var s_large = from_json[LazyUInt[origin_of(large)]](large)
...
def test_lazy_float() raises:
    # Test simple float
    var single = "123.456"
    var s_single = from_json[LazyFloat[origin_of(single)]](single)
    assert_equal(s_single.get(), 123.456)

    # Test negative float
    var negative = "-42.5"
    var s_negative = from_json[LazyFloat[origin_of(negative)]](negative)
    assert_equal(s_negative.get(), -42.5)

    # Test scientific notation
    var scientific = "1.23e4"
    var s_scientific = from_json[LazyFloat[origin_of(scientific)]](scientific)
...
def test_lazy_string() raises:
    # Test short string
    var short = '"short"'
    var s_short = from_json[LazyString[origin_of(short)]](short)
    ...
    var s_long = from_json[LazyString[origin_of(long_str)]](long_str)
    assert_equal(
        s_long.get(),
        (
    ...
    var s_escaped = from_json[LazyString[origin_of(escaped)]](escaped)
    assert_equal(s_escaped.get(), 'has "escaped" quotes')

    # Test escaped backslash
    var backslash = '"has \\\\ backslash"'
    var s_backslash = from_json[LazyString[origin_of(backslash)]](backslash)
    assert_equal(s_backslash.get(), "has \\ backslash")

    var mixed = '"foo \\" bar \\\\ baz"'
    var s_mixed = from_json[LazyString[origin_of(mixed)]](mixed)
